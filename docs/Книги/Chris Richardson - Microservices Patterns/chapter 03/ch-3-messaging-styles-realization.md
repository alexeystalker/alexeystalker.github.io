...
Этот стиль сочетает в себе стили "издатель/подписчик" и "[[request-response-messaging|запрос/ответ]]". Клиент публикует в канале "издатель-подписчик" сообщение с *каналом ответа* в заголовке. Потребитель записывает в канал ответа (типа "точка-точка") ответное сообщение с *идентификатором взаимодействия*. Клиент принимает ответы и сопоставляет их с запросом при помощи *идентфикатора взаимодействия*.