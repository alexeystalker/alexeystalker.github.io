...
> Далее описывается некая сущность, имеющая название trailer, как я понимаю, специфичная для gRPC. Возможные варианты перевода (прицеп, концевик, концевая метка), кажется, не совсем подходят (хотя схожая сущность header часто переводится как “заголовок”). Поэтому я решил оставить просто “трейлер”.
